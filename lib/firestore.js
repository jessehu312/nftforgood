...
export async function createUser(uid, data) {
  ...
  const goodCoins = query.data().goodCoins ?? 10;
  let payload = { uid, ...data, goodCoins };
...
export async function addNewCollectible(form) {
  const resp = await axios.post('api/proxy', {
    ...
    imgurl: form.file,
    secrethash: process.env.NEXT_PUBLIC_BACKEND_SECRET
  });
  ...
  const payload = {
    ...
    fiatPrice: form.price && form.checked ? form.price + 'ETH' : '',
    img: form.file,
    inStock: 1,
    name: form.name,
  ...
  // add item to chainlink blockchain

  await axios.post('/api/proxy', {
    action: 'deploynft',
    creator: form.creatorId,
    currowner: form.creatorId,
    name: form.name,
    description: form.desc,
    image: form.file
  });
